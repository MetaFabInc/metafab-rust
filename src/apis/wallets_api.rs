...
/// struct for typed errors of method [`create_wallet_signature`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum CreateWalletSignatureError {
    Status400(String),
    Status401(String),
...
pub async fn create_wallet_signature(configuration: &configuration::Configuration, wallet_id: &str, x_wallet_decrypt_key: &str, create_wallet_signature_request: crate::models::CreateWalletSignatureRequest) -> Result<String, Error<CreateWalletSignatureError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/v1/wallets/{walletId}/signatures", local_var_configuration.base_path, walletId=crate::apis::urlencode(wallet_id));
    ...
    local_var_req_builder = local_var_req_builder.json(&create_wallet_signature_request);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<CreateWalletSignatureError> = serde_json::from_str(&local_var_content).ok();
...
pub async fn get_wallet(configuration: &configuration::Configuration, wallet_id: &str) -> Result<crate::models::WalletModel, Error<GetWalletError>> {
    let local_var_configuration = configuration;
