...
/// struct for typed errors of method [`get_currency_balances`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetCurrencyBalancesError {
    Status400(String),
    UnknownValue(serde_json::Value),
}
...
pub async fn get_currency_balances(configuration: &configuration::Configuration, currency_id: &str, address: Option<&str>, wallet_id: Option<&str>) -> Result<f32, Error<GetCurrencyBalancesError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetCurrencyBalancesError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
